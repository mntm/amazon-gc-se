...
describe("validateInputJson", () => {
  ...
  test("Invalid amount throws error", () => {
    ...
    badAmounts.forEach((amount) =>
      expect(() => validateInputJson({ ...request, amount })).toThrowError()
    );
  });
  ...
  test("Valid strings", () => {
    ...
    stringFields.forEach((field) => {
      goodStrings.forEach((string) =>
        expect(() =>
          validateInputJson({ ...request, [field]: string })
        ).not.toThrowError()
      );
    });
  });
  ...
  test("Non-string value throws error", () => {
    ...
    stringFields.forEach((field) =>
      badStrings.forEach((badString) =>
        expect(() =>
          validateInputJson({ ...request, [field]: badString })
        ).toThrowError()
      )
    );
  });
  ...
  test("Valid enums", () => {
    for (let field in enums) {
      enums[field].forEach((value) =>
        expect(() =>
          validateInputJson({ ...request, [field]: value })
        ).not.toThrowError()
      );
    }
  });
  ...
  test("Invalid enum throws error", () => {
    for (let field in enums) {
      ...
      expect(() =>
        validateInputJson({ ...request, [field]: values.join("") })
      ).toThrowError();
    }
  });
});
